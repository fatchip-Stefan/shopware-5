...
class Shopware_Controllers_Frontend_MoptAjaxPayone extends Enlight_Controller_Action implements CSRFWhitelistAware
{
    ...
    public function createApplePaySessionAction()
    {
        ...
        if ($config['applepayNewAuthProcess'] === 0) {
            ...
            $auth = $paramBuilder->getAuthParameters();
            $call = new PayoneRequest(PayoneEnums::GenericpaymentAction_genericpayment, $auth);
            $params = [
                'add_paydata[action]' => PayoneEnums::APPLEPAY_INIT_SESSION,
                'add_paydata[display_name]' => Shopware()->Config()->shopname,
                'add_paydata[domain_name]' => 'apldemo-mass.patsy.dev',
            ...
            $rerspo = $call->request(PayoneEnums::GenericpaymentAction_genericpayment, $params);
